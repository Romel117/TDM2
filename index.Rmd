...
author: "Romel Gilberto Cruz Dominguez"
...
samp<- sq*ft1*ft2*ft3
highchart() %>% hc_add_series(cbind(t,samp), name="Sampled", color = "#7eb09b") %>% hc_add_series(cbind(t, ft1), dashStyle = "Dash", name="Original", color = "#d7816a") %>%  hc_add_series(cbind(t, ft2), dashStyle = "Dash", name="Original 2", color = "#f0cf65") %>% hc_add_series(cbind(t, ft3), dashStyle = "Dash", name="Original 3", color = "#93b5c6") %>% hc_title(text="Muestreo de señal con filtro", useHTML=T) %>% hc_add_theme(hc_theme_monokai()) %>% hc_xAxis(title=list(text="Tiempo")) %>% hc_yAxis(title = list(text="Valores", useHTML=T), min=-1.8, max = 1.8, allowDecimal = T) %>% hc_chart(zoomType="xy")
```
...
highchart() %>% hc_add_series(cbind(t,5*rec), name="Recovered", color = "#7eb09b") %>% hc_add_series(cbind(t, ft1), dashStyle = "Dash", name="Original 1", color = "#d7816a") %>%  hc_add_series(cbind(t, ft2), dashStyle = "Dash", name="Original 2", color = "#f0cf65") %>% hc_add_series(cbind(t, ft3), dashStyle = "Dash", name="Original 3", color = "#93b5c6") %>% hc_title(text="Recuperación de señal despues de filtro", useHTML=T) %>% hc_add_theme(hc_theme_monokai()) %>% hc_xAxis(title=list(text="Tiempo")) %>% hc_yAxis(title = list(text="Valores", useHTML=T), min=-1.8, max = 1.8, allowDecimal = T) %>% hc_chart(zoomType="xy")
```
...
#Graficación de las variables.
highchart() %>% hc_add_series(cbind(t,5*rec), name="Recovered", color = "#7eb09b") %>% hc_add_series(cbind(t, samp1), dashStyle = "Dash", name="Original 1", color = "#d7816a") %>%  hc_add_series(cbind(t, ft2), dashStyle = "Dash", name="Original 2", color = "#f0cf65") %>% hc_add_series(cbind(t, ft3), dashStyle = "Dash", name="Original 3", color = "#93b5c6") %>% hc_title(text="Recuperación de señal", useHTML=T) %>% hc_add_theme(hc_theme_monokai()) %>% hc_xAxis(title=list(text="Tiempo")) %>% hc_yAxis(title = list(text="Valores", useHTML=T), min=-1.8, max = 1.8, allowDecimal = T) %>% hc_chart(zoomType="xy")
```
